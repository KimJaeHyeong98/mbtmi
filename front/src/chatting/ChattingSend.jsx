...
const ChattingSend = () => {
  ...
  const getProfileSrc = (msg) => {
    // 내가 보낸 메시지(sent)면 프로필 필요 없음
    if (msg.type === "sent") return null;

    // received 메시지일 때 room.opponent_pic 사용
    return room?.opponent_pic
      ? `http://localhost:8080/uploads/${room.opponent_pic}`
      : profileimage; // 기본 이미지
  };
      ...
      <Messages>
        {messages.map((msg) => (
          <ChatBlock key={msg.id} type={msg.type}>
            {msg.type === "received" && (
              <ProfileImage src={getProfileSrc(msg)} alt={msg.name} />
            )}
